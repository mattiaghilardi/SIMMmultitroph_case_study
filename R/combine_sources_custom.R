#' Combine sources from a finished MixSIAR model (\emph{a posteriori}) modified from MixSIAR
#'
#' \code{combine_sources_custom} aggregates the proportions from multiple sources.
...
#' prior into an informative one. Because of this, \code{combine_sources_custom}
#' automatically generates a message describing this effect and, optionally, a figure
...
#' @param plot_prior logical, whether to plot the original, aggregated, and generalist priors (default = FALSE)
#'   
...
#' combined <- combine_sources_custom(jags.1, mix, source, alpha.prior=alpha, 
...
#' @export
#' 
combine_sources_custom <- function(jags.1, mix, source, alpha.prior=1, groups, plot_prior=FALSE){
