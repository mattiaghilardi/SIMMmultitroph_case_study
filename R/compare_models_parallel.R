...
#' 
#' Modified from MixSIAR to compare models fitted with parallel chains.
#' \code{compare_models_parallel} uses the \href{https://CRAN.R-project.org/package=loo}{'loo' package}
...
#' @param weights A string; the method used for obtaining model weights. One of "stacking" (default), 
#' "pseudobma", or "Akaike" (calculated as Akaike weights; p.75 Burnham & Anderson 2002).
#' See [loo::loo_model_weights()] for details on "stacking" and "pseudobma" weights.
#'   
...
#'  \item \code{weight}: relative support for each model. Interpretation: "an estimate of the probability that the model will make the best predictions on new data, conditional on the set of models considered" (McElreath 2015).
#' }
#'
...
#' @export
#' 
compare_models_parallel <- function(x, loo = TRUE, weights = c("stacking", "pseudobma", "Akaike")){
  
  if(class(x)!="list"){
    stop(paste("x is not a 'list'.
  Retry using e.g. 'compare_models(x=list(jags.1, jags.2))'.",sep=""))    
  }
  if(!"rjags" %in% class(x[[1]])){
...
'compare_models(x=list(jags.1, jags.2))'.\n\n")    
  ...
  if(loo){ # LOO
    LOOic <- sapply(loo.list, function(x) x$estimates["looic", "Estimate"])
    se_LOOic <- sapply(loo.list, function(x) x$estimates["looic", "SE"])
  } else { # WAIC
    LOOic <- sapply(loo.list, function(x) x$estimates["waic", "Estimate"])
    se_LOOic <- sapply(loo.list, function(x) x$estimates["waic", "SE"])    
  }
  dLOOic <- LOOic - min(LOOic)
  if (weights == "Akaike") {
    weight <- exp(-dLOOic/2) / sum(exp(-dLOOic/2))
  } else {
    weight <- loo::loo_model_weights(loo.list, method = weights)
  }
  ...
  for(i in 1:(n.mods-1)){
    for(j in (i+1):n.mods){
      dSE.mat[i,j] <- 2*loo::loo_compare(x=list(loo.list[[i]], loo.list[[j]]))[2, 2] # x2 to put on IC/deviance scale
      dSE.mat[j,i] <- dSE.mat[i,j]
    }#j
  }#i  
  dSE <- dSE.mat[, which(dLOOic==0)] # find the model with dLOOic = 0, use that col of dSE.mat
  
  # Produce data frame ordered by LOOic/WAIC
  result <- data.frame(Model = mod.names, 
                       LOOic = LOOic, 
                       se_LOOic = se_LOOic, 
                       dLOOic = dLOOic, 
                       se_dLOOic = dSE, 
