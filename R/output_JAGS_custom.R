#' Process mixing model output from JAGS modified from MixSIAR
#'
#' \code{output_JAGS_custom} processes the mixing model output, prints and saves (in the
#' working directory):
#' \itemize{
#'  \item diagnostics
...
#' }
#'
#' \code{output_JAGS_custom} adds two output options to \code{MixSIAR::output_JAGS},
#' \code{sup_post_resid} and \code{sup_cont_eff}.
#' 
...
#'    \item \code{sup_post_resid}: Suppress residual posterior density plot output in R? Default = \code{FALSE}
#'    \item \code{sup_cont_eff}: Suppress continuous effect plot output in R? Default = \code{FALSE}
#'   }
#'   
...
#' @export
#'   
output_JAGS_custom <- function(jags.1, mix, source, output_options=list(
  ...
  sup_post_resid = FALSE,                 # Suppress residual posterior density plot in R?
  sup_cont_eff = FALSE)){                 # Suppress continuous effect plot in R?
