#' Plot biotracer data (2-D) modified from MixSIAR
#'
...
#' @param linewidth Size or source errorbars
#' @param text_size Size or source text
#' @param point_size Size of consumer points
#' @param color_sources Color used for sources text, points and lines
#' 
#' @seealso \code{\link{plot_data}}
plot_data_two_iso_custom <- function(isotopes, 
                                     mix,
                                     source,
                                     discr,
                                     filename,
  ...
  g <- ggplot2::ggplot(data = df,ggplot2::aes(x = x, y = y)) +
    ...
    ggplot2::theme_bw() +
    ggplot2::theme(legend.position.inside = c(0, 1), 
