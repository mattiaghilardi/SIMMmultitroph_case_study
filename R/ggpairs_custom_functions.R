...
diag_custom <- function(data, mapping, xtext = -Inf, ytext = Inf, ...) {
  ...
  ggplot2::ggplot(data = data, mapping = mapping, ...) +
    ...
    ggplot2::annotate("text", label = ggplot2::as_label(mapping$x), 
                      x = xtext, y = ytext, ...) +
    ggplot2::theme_bw() +
...
point_custom <- function(data, mapping, linecolor = "black", ...) {
  ...
  ggplot2::ggplot(data = data, mapping = mapping, ...) +
    ggplot2::geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = linecolor) +
    ggplot2::geom_point(...) + 
    ggplot2::theme_bw() +
...
cor_custom <- function(data, mapping, main_model, sub_models = NULL,
  ...
  extract_cor <- function(model) {
    brms::as_draws_df(model) |> 
      dplyr::select(dplyr::starts_with("rescor")) |> 
      tibble::rownames_to_column(".draw") |> 
      tidyr::pivot_longer(cols = -.draw) |> 
      dplyr::mutate(name = gsub("rescor__",  "", name)) |>
      tidyr::separate_wider_delim(cols = name, delim = "__", names = c("TPa", "TPb")) |> 
      dplyr::filter((TPa == a & TPb == b) | (TPa == b & TPb == a)) |> 
      dplyr::group_by(TPa, TPb) |> 
      ggdist::median_qi(.width = 0.95) |> 
      dplyr::mutate(label = paste0("\u03C1 : ",
                                   round(value, digits), 
  ...
  if (is.null(sub_models)) {
    ...
    GGally::ggally_text(label = label, yP = 0.9, ...) +
      ggplot2::geom_text(data = sub_models, 
                         ggplot2::aes(x = 0.5, y = yPos, label = label, color = .data[[group_var]]),
                         ...) +
      ggplot2::theme_bw() +
      ggplot2::theme(panel.grid = ggplot2::element_blank())
  }
